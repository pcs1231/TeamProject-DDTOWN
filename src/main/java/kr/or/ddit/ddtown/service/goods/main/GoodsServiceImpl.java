...
public class GoodsServiceImpl implements IGoodsService {
			...
			for (goodsVO items : itemList) {
				...
				try {
                    ...
                    if (options != null && !options.isEmpty()) {
                        ...
                        // ### 이 부분을 수정합니다! ###
                        // 현재 items.setDefaultOptionNo(null); 로 되어 있는 부분을
                        // items.setDefaultOptionNo(0); 으로 변경합니다.
                    ...
                    // ### 이 부분도 수정합니다! ###
                    // items.setDefaultOptionNo(null); 로 되어 있는 부분을
                    // items.setDefaultOptionNo(0); 으로 변경합니다.
	...
	public goodsVO getGoodsDetail(int goodsNo) throws Exception {
		...
		goodsVO items = mapper.getGoodsDetail(goodsNo); //DB에서 기본 상품 정보 조회
		...
		if(items != null) {
			...
			if(items.getFileGroupNo() != null && items.getFileGroupNo() > 0) {
				...
				try {
					imageFiles = fileService.getRepresentativeFileByGroupNo(items.getFileGroupNo());
					items.setRepresentativeImageFile(imageFiles); //★대표 이미지 객체 설정!
			...
			// <-- 이 부분에 상품 상세 조회 시 기본 옵션 번호 설정 로직 추가 -->
            try {
                List<goodsOptionVO> options = mapper.optionList(items.getGoodsNo());
	...
	public void retrieveUserGoodsList(PaginationInfoVO<goodsVO> pagingVO) {
        ...
        // PaginationInfoVO의 setTotalRecord 메서드를 호출하여 totalPage 계산 및 설정
    ...
    public List<ArtistGroupVO> getArtistGroups() {
        // TODO Auto-generated method stub 대신 실제 로직을 작성합니다.
        // IGoodsMapper를 통해 데이터베이스에서 모든 아티스트 그룹 목록을 조회합니다.
