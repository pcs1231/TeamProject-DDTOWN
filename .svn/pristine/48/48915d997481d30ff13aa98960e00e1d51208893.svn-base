...
public class CartServiceImpl implements ICartService {
	...
	public int updateCartQuantity(GoodsCartVO cart) {
	 ...
	 public List<GoodsCartVO> getCartItemsByCartNos(List<Integer> cartNoList, String username) throws Exception {
	     ...
	     // 1. 매퍼를 통해 fileGroupNo를 포함한 기본 장바구니 항목들 조회
	     List<GoodsCartVO> orderItems = cartMapper.getCartItemsByCartNosAndUser(params);

	     // 2. 각 항목에 대해 이미지 URL 및 상품 상태 코드 처리 (getCartItemsUsername과 유사하게)
	     if (orderItems != null && !orderItems.isEmpty()) {
	         for (GoodsCartVO item : orderItems) {
	             ...
	             // 상품 상태 코드 로직 (필요시 GoodsMapper를 통해 실제 최신 상태를 가져오거나, GoodsCartVO에 이미 있는 goodsStatCode를 활용)
	             // 현재 쿼리에서 goodsStatCode를 가져오므로, 이를 기준으로 변환 로직만 추가
	 ...
	 @Override
	    ...
	    public void processCartAfterPayment(String username, List<OrderDetailVO> paidOrderDetails, boolean isFromCart) {
	        ...
	        // ★★★ isFromCart 플래그를 사용하여 장바구니 처리 로직을 조건부로 실행 ★★★
	        if (isFromCart) {
	            ...
	            int processedCount = 0; // 실제로 처리된 장바구니 항목 수 (선택 사항)
	            ...
	            for (OrderDetailVO detail : paidOrderDetails) {
	                ...
	                GoodsCartVO currentCartItem = cartMapper.selectCartItemByGoodsInfo(selectParams); // Map<String, Object>를 파라미터로 받는 메서드 사용
	        ...
	        // 재고 감소 로직은 장바구니 구매든 바로 구매든 항상 실행되어야 합니다.
	        // 이 위치에 재고 감소 로직을 추가하는 것이 좋습니다.
	        // 예를 들어:
	        // for (OrderDetailVO detail : paidOrderDetails) {
	        //    goodsStockMapper.decreaseStock(detail.getGoodsNo(), detail.getGoodsOptNo(), detail.getOrderDetQty());
	        //    log.info("재고 감소 처리 완료 - 상품No: {}, 옵션No: {}, 수량: {}", detail.getGoodsNo(), detail.getGoodsOptNo(), detail.getOrderDetQty());
	        // }
	    }
