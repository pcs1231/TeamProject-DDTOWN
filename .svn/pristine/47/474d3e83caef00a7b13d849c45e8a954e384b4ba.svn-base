...
public class OrderRestController {
    ...
    public ResponseEntity<Map<String, Object>> kakaoPayReady(
        ...
        if(payload.get("totalAmount") instanceof Integer) {
        	totalAmount = (Integer) payload.get("totalAmount"); // 클라이언트에서 받은 총 금액
        }else {
    ...
    public ResponseEntity<Map<String, Object>> cancelOrder(
        ...
        try {
            ...
            if (order.getOrderDetailList() != null && !order.getOrderDetailList().isEmpty()) {
                ...
                // 만약 goodsNo가 필수라면, 여기서 예외를 발생시키거나 적절한 기본값을 설정해야 합니다.
                // 예: throw new IllegalStateException("주문 상세 정보가 없어 취소할 수 없습니다.");
                // 예: orderCancelVO.setGoodsNo(0); // 0 또는 특정 기본 상품 번호
            }
            // ★★★ GOODS_NO 설정 끝 ★★★
